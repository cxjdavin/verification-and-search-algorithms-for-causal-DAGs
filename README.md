...
For non-atomic interventions, we know (Lemma 30) that $\nu_k(G^*) \geq \lceil \nu_1(G^*)/k \rceil$, so we use $\lceil \nu_1(G^*)/k \rceil$ as the denominator of the competitive ratio computation. While the competitive ratio increases as $k$ increases is expected (Theorem 16) but the actual **number of interventions** used decreases as $k$ increases.
